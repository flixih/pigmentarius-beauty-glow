...
const ContactSection = () => {
  ...
  const inputCls = "w-full px-4 py-3 rounded-xl text-sm text-ink placeholder-white/30 outline-none transition-all duration-200 border border-warm focus:border-white/20";
  const inputStyle = { background: "rgba(26,20,16,0.04)", backdropFilter: "blur(8px)" };

  return (
    <section id="contacto" className="py-20 md:py-32 relative" style={{ background: "var(--cream-alt)" }}>
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-14">
          ...
          <h2 className="font-serif text-4xl md:text-6xl font-bold text-ink mb-3">
            {t("contact_title1")}{" "}
          ...
          <p className="text-ink/40 text-sm max-w-sm mx-auto">{t("contact_desc")}</p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
          ...
          <div className="space-y-5">
            {[
              { icon: MapPin,  title: t("contact_address"), content: <>Plaza del Valle Mall, Suite 1<br/>Añasco, Puerto Rico 00610<br/><a href="https://www.google.com/maps/dir/?api=1&destination=18.2992483,-67.1669118" target="_blank" rel="noopener noreferrer" className="text-xs mt-1 inline-block hover:text-ink transition-colors" style={{ color: "hsl(330 85% 60%)" }}>{t("contact_directions")}</a></> },
              { icon: Phone,   title: t("contact_phone"),   content: <a href="tel:7878261684" className="text-lg font-semibold hover:text-ink transition-colors" style={{ color: "hsl(330 85% 60%)" }}>(787) 826-1684</a> },
              { icon: Clock,   title: t("contact_hours"),   content: <><div className="flex justify-between text-sm text-ink/50 mb-1"><span>{lang==="es"?"Mar – Vie":"Tue – Fri"}</span><span>9:00 AM – 5:00 PM</span></div><div className="flex justify-between text-sm text-ink/50 mb-1"><span>{lang==="es"?"Sábado":"Saturday"}</span><span>9:00 AM – 5:00 PM</span></div><div className="flex justify-between text-sm text-ink/30"><span>{lang==="es"?"Dom & Lun":"Sun & Mon"}</span><span>{lang==="es"?"Cerrado":"Closed"}</span></div></> },
            ].map((item) => (
              <div key={item.title} className="flex items-start gap-4 p-4 rounded-xl border border-warm" style={{ background: "rgba(26,20,16,0.03)" }}>
                <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 border border-warm" style={{ background: "rgba(255,255,255,0.05)", color: "hsl(330 85% 60%)" }}>
                  <item.icon size={16} />
                </div>
                <div>
                  <p className="text-ink/60 text-xs font-semibold tracking-widest uppercase mb-1">{item.title}</p>
                  <div className="text-ink/70 text-sm">{item.content}</div>
                </div>
              </div>
            ))}
            ...
            <div className="flex gap-3 pt-2">
              {[
                ...
                <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-2 px-4 h-10 rounded-full text-xs font-semibold text-ink/60 hover:text-ink border border-warm hover:border-white/20 transition-all duration-300" style={{ background: "rgba(26,20,16,0.04)" }}>
          ...
          {/* Call to action */}
          <div className="rounded-2xl p-6 border border-warm flex flex-col items-center justify-center text-center" style={{ background: "rgba(26,20,16,0.03)", backdropFilter: "blur(20px)" }}>
            <div className="w-16 h-16 rounded-full flex items-center justify-center mb-6 border border-warm" style={{ background: "rgba(255,255,255,0.05)", color: "hsl(330 85% 60%)" }}>
              <Phone size={28} />
            </div>
            <h3 className="font-serif text-2xl font-semibold text-ink mb-3">{lang === "es" ? "Llámanos" : "Call Us"}</h3>
            <p className="text-ink/40 text-sm mb-6 max-w-xs">{lang === "es" ? "Llámanos para reservar tu cita o resolver cualquier duda." : "Call us to book your appointment or ask any questions."}</p>
            <a href="tel:7878261684"
              className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-full text-lg font-semibold text-ink transition-all duration-300"
